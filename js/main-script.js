...
$(document).ready(function(){
	...
	$('#newsletter').submit(function(e){
		...
		if($(this).find('input[type="text"]').val() == ''){
			...
			if($(this).find('span.error').length==0){
				$(this).append('<span class="error">Te rog, completeaza adresa de email</span>');
			}else{
				$(this).find('span.error').text('Te rog, completeaza adresa de email');
			}
	...
	$('#contact-form').submit(function(e){
		...
		if(error == true){
			...
			if($(this).find('span.error').length==0){
				$(this).append('<span class="error">Te rog, completeaza toate campurile!</span>');
			}else{
				$(this).find('span.error').text('Te rog, completeaza toate campurile!');
			}
